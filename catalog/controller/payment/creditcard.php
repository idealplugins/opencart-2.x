...
class ControllerPaymentCreditcard extends Controller 
    ...
    public function storeTxid ($method, $txid, $order_id) 
    {
        $sql = "INSERT INTO `".DB_PREFIX."creditcard` SET ".
               ...
               "`creditcard_txid`='".$this->db->escape($txid)."' ON DUPLICATE KEY UPDATE `creditcard_txid`='".$this->db->escape($txid)."', creditcard_response=null";
        $this->db->query ($sql);
    }
    ...
    public function callback() 
    {
        ...
        if ($targetPay->getPaidStatus() || $this->config->get('creditcard_test')) {
            ...
            $order_status_id = $this->config->get('creditcard_canceled_status_id');
