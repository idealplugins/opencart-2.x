...
class ControllerPaymentpaysafecard extends Controller {
	...
	protected function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/paysafecard.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/paysafecard.tpl';
			} else {
			$this->template = 'default/template/payment/paysafecard.tpl';
			}
    ...
    public function storeTxid ($method, $txid, $order_id) {
    	...
    	$sql = "INSERT IGNORE INTO `".DB_PREFIX."paysafecard` SET ".
	...
	public function send() {
        ...
        elseif ($order_info['total'] > 150) {
            $this->log->write("Het bedrag is te hoog, maximaal 150 euro");
            $json['error'] = "Amount too large, max. 150 euro";
        } else {               

			$targetPay = new TargetPayCore ("WAL", $rtlo, "d949975cfdf8b2f037d393be79da93cb", "nl", false);
	...
	public function callback() {
		...
		$targetPay = new TargetPayCore ("MRC", $rtlo, "d949975cfdf8b2f037d393be79da93cb", "nl", false);
		$targetPay->checkPayment ($targetPayTx["paysafecard_txid"]);

		$order_status_id = $this->config->get('paysafecard_order_status_id');
        if (!$order_status_id) {
